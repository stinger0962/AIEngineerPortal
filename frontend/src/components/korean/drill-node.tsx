...
import type { DrillContent } from "@/lib/korean/types";
  ...
  return (
    <div className="space-y-5">
      ...
      <div className={`k-card rounded-3xl p-6 ${miss ? "k-pop" : ""}`} style={miss ? { borderColor: "#b9532b" } : undefined}>
        ...
        <div className="mb-4 mt-3 flex min-h-[64px] items-center">
          {item.type === "match" ? (
            <p className="font-kr-serif text-4xl text-ink">{item.ko}</p>
          ) : (
            <button
        ...
        <p className="mb-2 text-[11px] uppercase tracking-[0.18em] text-ink/40">
          {item.type === "match" ? "뜻을 고르세요 · Pick the meaning" : "들은 것을 고르세요 · Pick what you heard"}
        </p>
