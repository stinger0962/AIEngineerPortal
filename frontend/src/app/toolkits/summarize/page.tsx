    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <div className="flex items-center gap-3 mt-1">
          <div className="flex-shrink-0 w-11 h-11 rounded-[14px] bg-gradient-to-br from-[#e0ad55] via-[#c0892e] to-[#9c6a18] text-white flex items-center justify-center shadow-[0_6px_16px_-6px_rgba(192,137,46,0.6)]">
      ...
      <div className="lg:hidden flex gap-1.5 rounded-2xl bg-ink/5 p-1">
        <button
          ...
          className={`flex-1 flex items-center justify-center gap-1.5 rounded-xl py-2.5 text-sm font-semibold transition-colors ${mobileTab === "generate" ? "bg-white shadow-sm text-[#c0892e]" : "text-ink/50"}`}
        >
          <Sparkles className="w-4 h-4" strokeWidth={2} /> 生成
        </button>
        <button
          onClick={() => setMobileTab("library")}
          className={`flex-1 flex items-center justify-center gap-1.5 rounded-xl py-2.5 text-sm font-semibold transition-colors ${mobileTab === "library" ? "bg-white shadow-sm text-[#c0892e]" : "text-ink/50"}`}
        >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[2fr_3fr] rounded-[28px] border border-ink/10 bg-white/85 shadow-panel overflow-hidden lg:min-h-[600px]">
        ...
        <div className={`${mobileTab === "generate" ? "block" : "hidden"} lg:block relative overflow-hidden border-b lg:border-b-0 lg:border-r border-ink/10 bg-sand/20 p-5 lg:p-6`}>
          <Layers className="pointer-events-none absolute -right-6 -bottom-7 w-36 h-36 text-[#c0892e] opacity-[0.05]" strokeWidth={1.5} />
